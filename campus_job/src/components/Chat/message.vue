<template>
  <div id="message" v-scroll-bottom="sessions">
  	<ul v-if="this.sessions && this.sessions[this.user.sno+'#'+this.currentSession.sno]">
  		<li v-for="(entry,index) in this.sessions[this.user.sno+'#'+this.currentSession.sno]">
  			<p class="time">
  				<span>{{entry.date | time}}</span>
  			</p>
  			<div class="main" :class="{self:entry.self}">
					<p class="username">{{entry.fromNickname}}</p>
  				<img class="avatar" :src="entry.self ? 'http://localhost:8888/static/images/'+photos[user.sno] : 'http://localhost:8888/static/images/'+photos[currentSession.sno]" alt="">
